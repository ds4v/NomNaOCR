...
from utils import resize, draw_thresh_map
np.random.seed(2022)
...
class DBNetDataGenerator(tf.keras.utils.Sequence):
    def __init__(
        ...
        thresh_min=0.3, thresh_max=0.7, shrink_ratio=0.4, shuffle=True
    ):
    ...
    def __getitem__(self, batch_idx): # Generate 1 batch of data
        ...
        for batch_count, image_idx in enumerate(indexes_in_batch):
            ...
            for bbox in image_bboxes: # Generate gt and mask
                ...
                draw_thresh_map(bbox['points'], thresh_map, thresh_mask, self.shrink_ratio)

            batch_images[batch_count] = image / 255.0
